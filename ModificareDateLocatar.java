...
public class ModificareDateLocatar extends JFrame implements ActionListener {

	JTextField username, email, nume, cnp, prenume, localitate, judet, strada, nrAp, bloc, scara, nrPers;
	JPasswordField parola;
	JLabel label_user, label_parola, label_email, label_nume, label_cnp, label_prenume, label_localitate, label_judet,
			label_strada, label_nrAp, label_bloc, label_scara, label_nrPers;
	...
	ModificareDateLocatar(String curent) {
		this.setTitle("Modifica Locatar");
		this.persoanaCurenta = curent;
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == buton_signup) {
			...
			if (passText.length() >= 5 && userText.length() != 0 && cnpText.length() != 0 && numeText.length() != 0
					&& prenumeText.length() != 0 && emailText.length() != 0 && locText.length() != 0
					&& judText.length() != 0 && stradaText.length() != 0 && nrApText.length() != 0
					&& blocText.length() != 0 && scaraText.length() != 0 && nrPersText.length() != 0) {
				ModificaDate(userText, passText, cnpText, numeText, prenumeText, emailText, locText, judText,
	...
	public void ModificaDate(String user, String password, String cnp, String nume, String prenume, String email,
			String loc, String jud, String strada, String nrAp, String bloc, String scara, String nrPers) {
